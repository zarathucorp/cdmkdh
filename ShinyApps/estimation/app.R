library(shiny);library(DT);library(shinycustomloader);library(shinyhttr);library(shinyWidgets)
  ...
  observeEvent(data(), {
    ...
    output$typeOutcome <-renderUI({
      if (length(data()$fig.km) == 0){
        radioButtons("type_outcome", "Result select", choices = c("attrition", "psModelCoef", "outcomeModelTreatmentEstimate", "populationCounts", "outcomeCounts", "timeAtRisk", "Others"), selected = "attrition", inline = T)
      } else{
        radioButtons("type_outcome", "Result select", choices = c("attrition", "psModelCoef", "outcomeModelTreatmentEstimate", "populationCounts", "outcomeCounts", "timeAtRisk", "kaplanMeier", "Others"), selected = "attrition", inline = T)
      }
    ...
    if (input$type_outcome == "kaplanMeier"){
      ...
      output$model_outcome <- renderDT({
        ...
        if (input$type_outcome == "psModelCoef"){
          ...
          datatable(data.frame(Covariates = names(res[[input$type_outcome]]), coefficients = res[[input$type_outcome]]),  rownames = F, filter = "top", extensions= "Buttons", caption = "PS model coefficients", 
